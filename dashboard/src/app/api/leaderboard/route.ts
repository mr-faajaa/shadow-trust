...
export async function GET() {
  ...
  const agentsWithData = await Promise.all(
    AGENTS.map(async (agent) => {
      ...
      return {
        ...
        onChain: {
          balance: onChainData.balance.toFixed(4),
  ...
  return NextResponse.json({
    ...
    dataSource: isConnected ? 'solana-mainnet' : 'mock'
  })
}
