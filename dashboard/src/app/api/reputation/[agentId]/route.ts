...
// Agent wallet mapping (in production, this would be in a database)
...
export async function GET(
  ...
  // Get agent's wallet (if known)
  const walletAddress = AGENT_WALLETS[agentId]
  
  // Fetch on-chain data (or mock if no RPC configured)
  const onChainData = await solana.getAgentData(agentId, walletAddress)
  
  // Calculate reputation based on on-chain activity
  ...
  const response = {
    ...
    trend: calculateTrend(agentId),
    attestations: onChainData.transactions,
    onChainData: {
      balance: `${onChainData.balance.toFixed(4)} SOL`,
      transactions: onChainData.transactions,
      lastActive: onChainData.lastActive.toISOString(),
      programs: onChainData.programs
    },
    lastUpdated: new Date().toISOString(),
    dataSource: solana.isConnected() ? 'solana-mainnet' : 'mock'
  }
...
function calculateReputation(onChainData: any, agentId: string) {
  ...
  const overall = Math.floor(
    breakdown.taskCompletion * BREAKDOWN_WEIGHTS.taskCompletion +
    breakdown.paymentHistory * BREAKDOWN_WEIGHTS.paymentHistory +
    breakdown.identityVerification * BREAKDOWN_WEIGHTS.identityVerification +
    breakdown.onChainActivity * BREAKDOWN_WEIGHTS.onChainActivity
  )
