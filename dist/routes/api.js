...
function APIRouter(reputationService, solanaService) {
     ...
     */
    router.get('/payment/challenge', async (req, res) => {
        try {
            const amountLamports = parseInt(req.query.amount) || 1000; // Default 0.000001 SOL
            const recipient = req.query.recipient || 'ShadowTrustTreasury';
            // Create challenge
            const challenge = await solanaService.createPaymentChallenge(recipient, amountLamports, 5 // 5 minutes expiry
            );
